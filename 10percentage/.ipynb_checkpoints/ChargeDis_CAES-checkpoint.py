...
def ZeroProfit(x):
    ...
    cT=0.6/dT
    rat1=cT/dT
    dischargeeffi=np.sqrt(effi/rat1)
    ...
    print('Current ratio', rat)
    print('Current dischargeeffi', dischargeeffi)
    print('Current chargeeffi', chargeeffi)
        
    Discharge=network.links.loc['Discharge'].p_nom_opt*dischargeeffi
    
    # Save output variables that I want as a panda dataframe. 
    
    P=[power,energy,effi,network.stores.e_nom_opt.values, network.links.loc['Charge'].p_nom_opt,network.links.loc['Discharge'].p_nom_opt*dischargeeffi,fuel_cost,g,s,w,st,c,ww,ss,chargeeffi,factorc]

    df=pd.DataFrame([P],columns=['power','energy','effi','storage','charge','discharge','fuel_cost','Gasuse','solar','wind','discharge','charge','Wind','Solar','chargeeffi','costfrac'])
...
B1=    df=pd.DataFrame([P],columns=['power','energy','effi','storage','charge','discharge','fuel_cost','Gasuse','solar','wind','discharge','charge','Wind','Solar','chargeeffi','costfrac'])
effi=0.6
for fuel_price in [100]:   
    for rat in [0.097,0.333,0.5, 0.6]:
        for energy in [1000, 2000, 5000,10000,20000, 30000]:
            A=101
            cc=300000
            while A>100:
                cc=cc+200000
                B=ZeroProfit([cc, energy, effi,fuel_price,rat])
                A=B[0]
                B1=B1.append(B[1])
...
B1.to_csv('CompressAir_ChargeDis.csv',index=False)