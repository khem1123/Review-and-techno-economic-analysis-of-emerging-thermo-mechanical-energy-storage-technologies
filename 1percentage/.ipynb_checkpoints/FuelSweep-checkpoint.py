...
P=[0,0,0,0,0,0,0 ]
B1=pd.DataFrame([P],columns=['power','energy','effi','storage','charge','discharge','Feul'])
...
while Store>10:
    ...
    fuel_cost = fuel_cost+1 # in €/MWh_th
    efficiency = 0.33
    ...
    Store=network.stores.e_nom_opt
    ...
    P=[power,energy,effi,network.stores.e_nom_opt.values, network.links.loc['Charge'].p_nom_opt,network.links.loc['Discharge'].p_nom_opt*dischargeeffi,fuel_cost]

    B1=pd.DataFrame([P],columns=['power','energy','effi','storage','charge','discharge','fuel_cost'])
    B1=B1.append(B1)
